...
enum Algorithm { FIFO }; // Extend as needed
Algorithm currentAlgo = FIFO;
...
Page evictPage(Algorithm algo) {
    Page victim = {"", -1};
    if (algo == FIFO && !pageQueue.empty()) {
...
void pageIn(Process& proc, int pageNum, Algorithm algo) {
    if (!isPageInMemory(proc.name, pageNum)) {
        ...
        if (freePageList.empty()) {
            evicted = evictPage(algo);
        }
        int frame = freePageList.front();
        freePageList.pop();
        memory[frame] = {proc.name, pageNum};
        if (algo == FIFO) pageQueue.push({proc.name, pageNum});
